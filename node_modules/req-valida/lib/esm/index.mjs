 ...
 */
export function validate({ location, data, isOptional }) {
        ...
        try {
            ...
            for (const key in data) {
                if (request[location][key] === undefined &&
                    !isOptional &&
                    !data[key].isOptional) {
                    next(createError(400, `'${key}' in request ${location} is missing`));
                }
            ...
            for (const key in request[location]) {
                if (!data[key]) {
                    next(createError(400, `Unexpected field '${key}' in request ${location}.'`));
                }
            }
            next();
        }
